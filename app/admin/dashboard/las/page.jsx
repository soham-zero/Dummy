...
export default function AdminLASPage() {
  ...
  // 📦 Fetch LAS data (ascending order)
  useEffect(() => {
    const getData = async () => {
      const fresh = await fetchLAS();
      setData(fresh);
  ...
  const handleAddRow = () => {
    const emptyRow = Object.fromEntries(
      Object.keys(lasSchema).map((key) => [key, null])
    );
    const maxExistingId = Math.max(
      0,
      ...data.map((r) => Number(r.id) || 0),
      ...newRows.map((r) => Number(r.id) || 0)
    );
    ...
    try {
      ...
      for (const row of data) {
        ...
        if (original && JSON.stringify(original) !== JSON.stringify(row)) {
          await supabase.from("las").update(row).eq("id", row.id);
        }
      }

      // Insert new rows (strip temporary IDs)
      if (newRows.length > 0) {
        const sanitizedRows = newRows.map(({ id, ...rest }) => rest);
        await supabase.from("las").insert(sanitizedRows);
      }

      // Delete rows
      if (deletedIds.length > 0) {
        await supabase.from("las").delete().in("id", deletedIds);
      }
  ...
  const commitEdit = (newValue) => {
    const fieldType = lasSchema[modalEditor.key].type;
    ...
    // Convert empty strings to null for numeric and text
    const valueToSet =
    ...
    if (modalEditor.isNewRow) {
      setNewRows((prev) => updater(prev));
    } else {
  ...
  return (
    <div className="p-8 bg-gray-50 min-h-screen">
      <h1 className="text-4xl font-bold mb-8 text-gray-800">LAS Admin Dashboard</h1>
      ...
      <div className="overflow-x-auto bg-white rounded-xl shadow-xl border border-gray-200">
        <table className="w-full border-collapse text-sm">
          <thead className="bg-gray-100">
            <tr>
              {Object.keys(lasSchema).map((key) => (
                <th
                  key={key}
                  className="px-4 py-3 border-b text-gray-700 max-w-[240px] truncate"
                >
                  {key}{" "}
        ...
        <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-50">
          <div className="bg-white rounded-lg shadow-xl p-6 w-[650px] max-h-[80vh] overflow-y-auto">
            <h3 className="text-lg font-semibold mb-4 text-gray-800 flex justify-between items-center">
              Editing Field:{" "}
              <span className="text-blue-600">{modalEditor.key}</span>
              <button
                onClick={closeEditor}
                  ...
                  <div key={i} className="flex items-center gap-2">
                    ...
                    <input
                      ...
                      onBlur={(e) => {
                        ...
                        // Run validation only after finishing typing
                        if (newKey === "") {
                          alert("⚠️ Key cannot be empty.");
                          return;
                        }
                        if (newKey !== k && newKey in newVal) {
                          alert("⚠️ Duplicate key detected!");
                          return;
                        }
