...
// MTF schema definition
const mtfSchema = {
  id: { type: "serial", editable: false },
  broker_name: { type: "text", editable: true },
  margin_requirement: { type: "number", editable: true },
  approved_stocks: { type: "number", editable: true },
  subscription_fee: { type: "number", editable: true },
  interest_slabs: { type: "json", editable: true },
  intraday_fee: { type: "text", editable: true },
  carry_fee: { type: "text", editable: true },
  pledge_unpledge_fee: { type: "number", editable: true },
  auto_square_off: { type: "number", editable: true },
  dp_charges: { type: "text", editable: true },
  unpaid_mtf_interest: { type: "number", editable: true },
  platform_rating: { type: "number", editable: true },
  feedback_rating: { type: "number", editable: true },
  cost_summary: { type: "json", editable: true },
};

export default function AdminMTFPage() {
  ...
  // ➕ Add Row
  const handleAddRow = () => {
    const emptyRow = Object.fromEntries(Object.keys(mtfSchema).map((k) => [k, null]));
    ...
    try {
      ...
      for (const row of data) {
        ...
        if (original && JSON.stringify(original) !== JSON.stringify(row)) {
          await supabase.from("mtf").update(row).eq("id", row.id);
        }
      }
      // Insert new rows
      if (newRows.length > 0) {
        const sanitized = newRows.map(({ id, ...rest }) => rest);
        await supabase.from("mtf").insert(sanitized);
      }
      // Delete rows
      if (deletedIds.length > 0) {
        await supabase.from("mtf").delete().in("id", deletedIds);
      }
  ...
  const commitEdit = (newValue) => {
    const fieldType = mtfSchema[modalEditor.key].type;
  ...
  if (loading)
    return <div className="flex justify-center items-center h-screen text-gray-600 text-lg">Loading MTF table...</div>;

  return (
    <div className="p-8 bg-gray-50 min-h-screen">
      <h1 className="text-4xl font-bold mb-8 text-gray-800">MTF Admin Dashboard</h1>
      ...
      <div className="overflow-x-auto bg-white rounded-xl shadow-xl border border-gray-200">
        <table className="w-full border-collapse text-sm">
          <thead className="bg-gray-100">
            <tr>
              {Object.keys(mtfSchema).map((key) => (
                <th key={key} className="px-4 py-3 border-b text-gray-700 max-w-[240px] truncate">
                  {key} <span className="text-xs text-gray-500">({mtfSchema[key].type})</span>
                </th>
              ))}
              <th className="px-4 py-3 border-b text-center text-gray-700 sticky right-0 bg-gray-100 z-10">
                Actions
              </th>
            </tr>
          </thead>
        ...
        <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-50">
          <div className="bg-white rounded-lg shadow-xl p-6 w-[650px] max-h-[80vh] overflow-y-auto">
            ...
            <div className="flex justify-end mt-6 gap-3">
              <button
                onClick={() => commitEdit(modalEditor.value)}
